<template>
  <div class="ve_container">
    ...
    <!-- 编辑组件 -->
    <users-edit v-show="false" @isUpdate="handleChildMessage" ref="UserEdit" />
  </div>
</template>
<script>
export default {
...
const handleDel = (row) => {
  proxy.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
    ...
    let params = { data: { id: row.id }, gether: "t_user_list" }
...
const getDataList = async () => {
  let param = { data: {}, gether: "t_user_list" }
  const { data } = await get_List(param);
...
const handleEdit = async (isEdit, row) => {
  instance.refs.UserEdit.dialogFormVisible = true
  instance.refs.UserEdit.rowJson = row
  instance.refs.UserEdit.isEdit = isEdit
}
